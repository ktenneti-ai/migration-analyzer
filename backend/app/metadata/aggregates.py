...
def compute_dashboard(data: ProjectData) -> dict:
    ...
    # No Teradata SQL/DDL ingestion pipeline exists yet, so teradata_views/
    # teradata_base_tables legitimately stay 0. But every table's
    ...
    # split for its own per-row detail.
    teradata_referenced = sum(
        1 for t in tables if t.source_hint and t.source_hint.startswith(TERADATA_SOURCE_HINT_PREFIX)
    )
