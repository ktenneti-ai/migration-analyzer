#!/usr/bin/env bash
# Runs every time the Codespace (re)starts: launches the backend and a
# production build of the frontend in the background so the forwarded ports
# are live as soon as the Codespace is ready.
#
# The frontend is served via `vite preview` (a built, static bundle) rather
# than `vite dev` — Vite's dev server ships relatively raw, unbundled ES
# module code plus its React Refresh/HMR bootstrap, which assumes a modern
# JS engine; this broke with a SyntaxError in Safari specifically (blank
# page) while working in Chrome once past Codespaces' own tunnel auth. The
# production build is properly transpiled/bundled by esbuild+rollup for
# broader compatibility, and a demo doesn't need hot-reload anyway.
cd "$(dirname "$0")/.."
...
(cd frontend && npm run build) >> "$LOG_DIR/frontend.log" 2>&1

echo "==> Starting frontend preview on :5180..."
(cd frontend && nohup npm run preview -- --host 0.0.0.0 --port 5180 --strictPort \
